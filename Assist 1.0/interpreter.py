...
class MyApp(QMainWindow):
    ...
    def keyPressEvent(self, event):
        if (event.key() == Qt.Key_Return or event.key() == Qt.Key_Enter):
            ...
            label = TypingLabel("Good Luck today", self)  # 수정된 부분
            label.setAlignment(Qt.AlignCenter)
            self.layout.addWidget(label)  # 클래스 변수로 설정된 layout 사용
            timer = QTimer(self)
            timer.singleShot(1000, self.quit)

        elif event.key() == Qt.Key_Up and self.second_show_on == True:
            self.clear()
            self.today += 1
            self.select = 0
        ...
        elif event.key() == Qt.Key_Down and self.second_show_on == True:
            self.clear()
            self.today -= 1
            self.select = 0
            ...
            with open('date.txt', 'r') as data:
                data = data.read()
                a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12 = reader.second_data(data, self.today)
                kl = a9.split(',')

            if len(kl) > self.select:
                self.select += 1
            self.second_show()
    ...
    def updateTime(self):
        ...
        self.status_bar.showMessage(current_date.toString(Qt.ISODate) + " " + current_time.toString("AP h:mm:ss"))

    def first_show(self):
        self.first_show_on = True

        label = TypingLabel(reader.first, self)
        label.setAlignment(Qt.AlignCenter)
    ...
    def second_show(self):
        ...
        if self.select == 0:
            datetype = TypingLabel_indepent_line(reader.super(self.today), self)
        elif self.select > 0:
            datetype = TypingLabel_indepent_line(reader.super(self.today, self.select), self)
        else:
            self.select = 0
            datetype = TypingLabel_indepent_line(reader.super(self.today), self)
        if self.select == 0:
    ...
    def process_input(self):
        # 입력 필드에서 텍스트 가져오기
        input_text = self.status_input.text()  # 앞뒤 공백 제거
...
class TypingLabel_indepent_line(QLabel):
    def __init__(self, text, parent=None):
        super().__init__(parent)
        self.setFont(QFont("NeoDunggeunmo", 14))
        self.setAlignment(Qt.AlignCenter)
        self.text = text.split('\n')  # 입력된 문자열을 줄 단위로 분할
        self.current_lines = [""] * len(self.text)
        self.typing_timer = QTimer(self)
        self.typing_timer.timeout.connect(self.updateText)
        self.multiplier = 1
        self.currentIndex = 0
    ...
    def updateText(self):
        if self.currentIndex < max(len(line) for line in self.text):
            for i, line in enumerate(self.text):
                if self.currentIndex < len(line):
                    if line[self.currentIndex] in ',!.\'?':
                        self.multiplier = 3
                    else:
                        self.multiplier = 1

                    self.current_lines[i] += line[self.currentIndex]
